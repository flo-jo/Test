...
export const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose, onSelectItem }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 bg-gray-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div
      ...
      <div
        ...
        className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden z-10 animate-in zoom-in-95 duration-150"
      >
        {/* Search Input Bar */}
        <div className="flex items-center px-4 py-3.5 border-b border-gray-100 gap-3">
          ...
          <input
            ...
            className="w-full text-base font-medium text-gray-900 placeholder-gray-400 bg-transparent focus:outline-none"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
          ...
          <kbd className="text-[11px] font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded border border-gray-200">
            ESC
          </kbd>
        </div>

        {/* Filter Tabs */}
        <div className="flex items-center gap-1.5 px-4 py-2 bg-gray-50/70 border-b border-gray-100 text-xs overflow-x-auto">
            ...
            filteredItems.map((item) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between p-3 rounded-2xl hover:bg-blue-50/60 transition-colors cursor-pointer group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center font-bold text-xs text-gray-700 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold bg-gray-100 px-1.5 py-0.5 rounded">
                          {item.exchange}
                        </span>
                      </div>
                      <div className="text-xs text-gray-500 truncate max-w-[240px] sm:max-w-xs">
                        {item.name}
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {recentSearches.map((symbol) => (
              <button
                ...
                className="bg-white border border-gray-200 hover:border-blue-400 text-gray-700 px-2 py-0.5 rounded-md font-mono text-[11px] transition-colors"
              >
                {symbol}
              </button>
            ))}
